...
-- Created: 22:00 ope, Sunday 05 August, 2012
...
comUnInitialize :: IO ()
comUnInitialize = prim_ComPrim_comUnInitialize
...
writeCOAUTHIDENTITY ptr (COAUTHIDENTITY user domain password flags) =
  let userLength = (fromIntegral (length user) :: Word32)
  in
  do
...
coCreateInstanceEx clsid pUnkOuter dwClsCtx pServerInfo pResults =
  let cmq = (fromIntegral (length pResults) :: Word32)
  in
  do
...
messagePump :: IO ()
messagePump = prim_ComPrim_messagePump

foreign import ccall "messagePump" prim_ComPrim_messagePump :: IO ()
postQuitMsg :: IO ()
postQuitMsg = prim_ComPrim_postQuitMsg
